...
export const createOrFindContactVerification = async (
	contactInfo: { email: string; phone: string },
	industryVerificationToken: string | null
) => {
	...
	if (
		...
		try {
			existingContactVerification = await prisma.contactVerification.update({
				...
				data: {
					industryVerificationTokenDetails: { connect: { token: industryVerificationToken } }
				}
			})
		} catch (err) {
...
export async function sendPhoneVerificationCode(phone: string, contactVerificationId?: string) {
	...
	if (contactVerificationId !== undefined) {
		await prisma.contactVerification.update({
			where: { id: contactVerificationId },
			data: { phoneVerificationCodeSentAt: new Date() }
		})
	}
}
...
export async function sendEmailVerificationCode(email: string, contactVerificationId: string) {
	...
	await prisma.contactVerification.update({
		where: { id: contactVerificationId },
		data: { emailVerificationCodeSentAt: new Date() }
	})
}
