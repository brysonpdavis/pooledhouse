...
const credentialsSchema = z.object({
	...
	code: z
		.string()
		.max(6, 'industry verification code is invalid')
		.nullable()
		.refine(async (token) => {
			if (token === null) {
				return true
			...
			const ivt = await prisma.industryVerificationToken.findFirst({
				where: { token, consumedByUser: null }
			})
...
export const actions: Actions = {
	default: async ({ request }) => {
		...
		const user = await prisma.user.findFirst({
			where: {
				OR: [{ email: validatedCredentials.data.email }, { phone: validatedCredentials.data.phone }]
			}
		})
		...
		const verification = await createOrFindContactVerification(
			validatedCredentials.data,
			validatedCredentials.data.code
		)
