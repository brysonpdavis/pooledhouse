...
export const POST: RequestHandler = async ({ params: { placeId } }) => {
	...
	const workplaceCompensationRatings = reviewScores.workplaceReviews
		.map((wr) => wr.compensationRating)
		.filter(notNull)
	...
	const experienceFnbRatings = reviewScores.experienceReviews
		.map((er) => er.fnbRating)
		.filter(notNull)
	...
	const experienceVibeRatings = reviewScores.experienceReviews
		.map((er) => er.vibeRating)
		.filter(notNull)
